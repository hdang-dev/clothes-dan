...
import { IFilters, IProduct } from "@/interfaces";
...
export default function Home() {
  ...
  return (
    <>
      <Header onClickLogo={() => setReset(reset + 1)} onSearch={(text) => debounceChangeSearchText(text)} reset={reset} />
      <Flex flexGrow="1" width="100%" justify="center" align="center">
        <Flex width="100%" height="100%" pt="30px" gap="8">
          ...
          <Flex flexGrow="1" direction="column" gap="3" justify={products?.length ? "start" : "center"} align={products?.length ? "start" : "center"}>
            {products ? <ProductList products={products} reset={reset} /> : <Spinner size="3" />}
          </Flex>
        </Flex>
      </Flex>
      <Footer />
